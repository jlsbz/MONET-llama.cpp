...
| 项目产出量化记录 | 统计 5 个关键代码文件、1 篇投稿论文、2 份技术交底书、2 份技术文档、12 张过程/投稿图片，并列出截图和论文中的主要结果 | [docs 目录](../docs/)、[scripts 目录](../scripts/)、[修改版 llama.cpp](../llama.cpp-monarch/) | 文件清单；论文第 5 节；`image-7.png`—`image-10.png` | 已部分填写 | 正式成果统计口径、是否还有未入库成果 |
...
| 1.项目产出—GGUF 转换 | `--monarch-dir`/`--monarch-dtype`；HF 层名映射；追加 L/R/perm extra tensor | [转换脚本](../llama.cpp-monarch/convert_hf_to_gguf_monarch.py)、中文项目说明手册 | 脚本第 52—212、379—396、518—532 行；手册第 6 节 | 已充分填写 | 成功生成 384 tensors 的 reader dump、GGUF 样例与哈希 |
...
| 1.项目产出—完整 graph/算子/kernel | 报告明确说明当前仓库未检索到 `GGML_OP_MONARCH_LINEAR`、forward graph 使用或 MONET 专用 CPU/ARM/GPU kernel | 当前 `llama.cpp-monarch/src` 与 `ggml` 源码；[中文项目说明手册](../docs/中文项目说明手册.md) | 全仓库精确检索；手册第 11—12 节把多项内容写为“应实现/需要增加” | 已部分填写 | 完整实现是否在其他分支/仓库；若无，应确定结项范围和遗留计划 |
...
| GGUF 扩展 | [`convert_hf_to_gguf_monarch.py`](../llama.cpp-monarch/convert_hf_to_gguf_monarch.py) | [GitHub](https://github.com/jlsbz/MONET-llama.cpp/blob/main/llama.cpp-monarch/convert_hf_to_gguf_monarch.py) | 从 `.pt` 读取 L/R/perm，追加 extra tensor，支持 f32/f16 | 仍保留 dense 权重；无 `--monarch-only` |
...
|---|---|---|---|
| 完整 runtime 状态 | 方案第 10.1 节称已完成 `llama.cpp/GGML` 自定义算子支持；论文称已实现分解、流式执行和块选择 | 当前代码仅确认拟合、GGUF extra tensor、模型字段和 loader；未发现 `GGML_OP_MONARCH_LINEAR`、graph 调用或专用 kernel；中文手册第 11—13 节仍以“应实现/需要增加/规划”描述 | 报告采用代码可核验边界，把完整 runtime 标为待确认，不直接认定已完成 |
